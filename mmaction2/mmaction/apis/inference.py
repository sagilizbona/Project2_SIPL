...
# def inference_recognizer(model, video, outputs=('backbone.layer1','backbone.layer2','backbone.layer3','backbone.layer4'), as_tensor=True, **kwargs):
def inference_recognizer(model, video, outputs=None, as_tensor=True, **kwargs):
    ...
    # tmp = returned_features.get('backbone.layer1').cpu()
    # tmp = tmp*255
    # tmp = np.array(tmp, dtype=np.uint8)
    # if np.ndim(tmp)>3:
    #     # assert tensor.shape[0] == 1
    #     tensor = tmp[0]
    # ret = PIL.Image.fromarray(tensor)
    # print (ret)
    # cv2.imshow(ret)
    ...
    selected_features = [score_tuples[265], score_tuples[525], score_tuples[516]]  # score_tuples[525],
    if outputs:
